...
# PLOT SIGNAL
plt.figure(1)
plt.plot(time[0:10000],voltplot[0:10000])
plt.grid('true')
plt.ylabel('Volt [V]')
plt.xlabel('Time [s]')
...
x = np.arange(0,l)
plt.figure(4)
plt.plot(time[0:10000],voltplot[0:10000],label='INPUT')
plt.plot(time[0:10000], OutputSignal[0:10000],label='OUTPUT')
plt.ylabel('Volt [V]')
plt.xlabel('Time [s]')
plt.grid('true')
...
if INPUT_1 == "calc":
    print(f' high pass passband edge : {Wp} Hz --> {wp} rad/s')
    print(f' high pass stopband edge : {Ws} Hz --> {ws} rad/s')
else:
